...
`extensions/background-commands.ts` 提供轻量后台命令：agent 用 `bg_run` 启动，
命令自行结束后会自动把退出状态和保留的输出插入主会话并唤醒 agent；
用 `bg_status` 查询运行中进度或查看历史，用 `bg_kill` 终止任务。
用户也可在 TUI 输入 `/bg <命令>`、`/jobs [任务 ID]`、`/kill <任务 ID>`。
启动后立即返回，命令在调用时的工作目录执行；标准输出和错误输出合并显示。
TUI 输入框上方的 `Background` 栏只在有活动任务时出现，每行仅显示任务 ID、运行中的命令和耗时；
最多显示最近 6 个活动任务，其余可用 `/jobs` 查看，全部结束后自动隐藏。
每个 Pi 进程最多跟踪 32 个任务，每个任务仅保留最近 64 KiB 输出。
任务记录只在当前 Pi 进程内有效，正常退出时会终止仍在运行的任务。
