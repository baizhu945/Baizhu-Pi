...
import { type ChildSessionPolicy, runInChildSessionContext } from "./child-context.js";
...
/** Names of orchestration tools subagents must NOT inherit. */
 ...
 *
 * Keep this path on the same resolver as the Agent tool and workflow host. The
 * previous duplicate implementation only understood `provider/modelId`, so a
 * direct runner call treated a valid bare pin such as `haiku` differently from
...
export function resolveDefaultModel(
  ...
  if (!configModel || configModel.trim().toLowerCase() === "inherit") return parentModel;
  const resolved = resolveModel(configModel, registry);
  return typeof resolved === "string" ? parentModel : resolved;
}
...
function forwardAbortSignal(session: AgentSession, signal?: AbortSignal): () => void {
  ...
  // AbortSignal does not replay an already-fired event to late listeners. This
  // occurs for queued starts and resumed runs whose parent was cancelled while
  // they waited; abort the newly-created/reused session immediately instead of
  // silently running a task that was already cancelled.
  if (signal.aborted) {
    session.abort();
    return () => {};
  }
...
export async function runAgent(
  ...
  const childLoaderPolicy: ChildSessionPolicy = {
  ...
  const childSessionPolicy: ChildSessionPolicy = {
    allowedTools: [...new Set([...toolNames, ...nestedToolNames, ...structuredToolNames])],
    allowExtensionTools: !noExtensions,
  };
  ...
  // Track turns for the live widget only. There is no turn-count cutoff: the
  // subagent runs until it reaches its own answer, is externally stopped, or
  // the provider/session fails.
  let turnCount = 0;
