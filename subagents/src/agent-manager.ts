...
/**
 * Default max concurrent foreground (blocking) agents — `0` = unlimited.
 *
 ...
 * goes, not how WIDE. Child fan-out remains bounded only by the background
 * concurrency pool and the configured nesting depth.
 */
...
export class AgentManager {
  ...
  constructor(
    ...
    this.maxConcurrent = Number.isSafeInteger(maxConcurrent) ? Math.max(1, maxConcurrent) : DEFAULT_MAX_CONCURRENT;
  ...
  spawn(
    ...
    if (pool !== undefined && !options.bypassQueue && !this.poolHasRoom(pool)) {
      ...
      try {
        ...
        // Queue/UI reporting is auxiliary; a throwing observer must not leave
        // the newly queued record stranded or turn a valid spawn into an error.
      }
      return id;
    }
   ...
   *
   * The listener is removed when the record starts or leaves the queue, so a
   * long-lived parent signal does not retain settled records through a closure.
   */
  private armQueuedAbort(id: string, signal?: AbortSignal): boolean {
    ...
    const record = this.agents.get(id);
    if (record) record.queuedAbortCleanup = () => signal.removeEventListener("abort", onAbort);
    return true;
  }
    ...
    const startup = this.startAgent(id, record, args).then(
      ...
      (err) => {
        ...
        if (queuedPool !== undefined) {
          ...
          record.completedAt = Date.now();
          if (!this.disposed) {
            try { this.onComplete?.(record); } catch { /* reporting must not strand the record */ }
          }
        } else {
  ...
  private async startAgent(
    ...
    // UI/lifecycle observers are optional side effects. A renderer or third-party
    // listener must not turn a started run into a startup rejection (which would
    // otherwise leak its claimed pool slot before `runAgent` owns settlement).
    try { this.onStart?.(record); } catch { /* observer failure must not strand the run */ }
    ...
    // A failure in the success-side cleanup must not fall through to the
    // promise's catch and release/notify the same run a second time.
    let runSettled = false;
    const settle = (guardCallback: boolean) => {
      if (runSettled) return;
      runSettled = true;
      this.settleRun(record, guardCallback, pool);
    };
    ...
    const promise = runAgent(ctx, type, prompt, {
      ...
      onToolActivity: (activity) => {
        ...
        try { options.onToolActivity?.(activity); } catch { /* UI observer */ }
      },
      onTurnEnd: (turnCount) => {
        try { options.onTurnEnd?.(turnCount); } catch { /* UI observer */ }
      },
      onTextDelta: (delta, fullText) => {
        try { options.onTextDelta?.(delta, fullText); } catch { /* UI observer */ }
      },
      onAssistantUsage: (usage) => {
        addUsage(record.lifetimeUsage, usage);
        try { this.onUsage?.(record, usage); } catch { /* usage observer */ }
        try { options.onAssistantUsage?.(usage); } catch { /* UI observer */ }
      },
      onCompaction: (info) => {
        record.compactionCount++;
        try { this.onCompact?.(record, info); } catch { /* compaction observer */ }
        try { options.onCompaction?.(info); } catch { /* UI observer */ }
      },
      nestedRuntime: {
        manager: this,
        parentAgentId: id,
      ...
      onSessionCreated: (session) => {
        ...
        try { options.onSessionCreated?.(session); } catch { /* UI observer */ }
      },
    })
      .then(async ({ responseText, session, failure, structuredJson, structuredRetried }) => {
        ...
        // Detached nested children are allowed to finish after a successful
    ...
    try { options.onSpawned?.(id); } catch { /* caller wiring must not strand the run */ }
  }
   ...
   * already delivered. Completion reporting is isolated from the run so an
   * event listener or renderer cannot create an unhandled rejection or prevent
   * the queue from draining.
   *
   * @param _guardCallback retained for call-site compatibility; both paths are
   *   guarded.
  ...
  /**
   * Stop owned children when the owner cannot process a result anymore. Detached
   * nested children intentionally survive a normal `completed` parent turn so
   * their automatic result can start a follow-up turn in the owner's session.
   * Explicit stop/provider failure still cancel the branch; record removal also
   * calls `abortOwnedChildren` directly.
   */
  private abortOwnedChildrenIfTerminated(record: Pick<AgentRecord, "id" | "status">): void {
    if (record.status !== "stopped" && record.status !== "aborted" && record.status !== "error") return;
  ...
  async resume(
    ...
    if (options?.isBackground) {
      ...
      if (occupiesPoolSlot(record) && !this.poolHasRoom("background")) {
        ...
        this.queue.push({
          id,
          pool: "background",
          // Keep the background slot occupied until the resumed turn settles.
          // `startResume()` returns that promise; resolving this entry as soon
          // as it merely *launches* the prompt would drain every queued resume
          // at once and defeat maxConcurrent.
          start: () => {
            try {
              return start();
            } catch (err) {
    ...
    // Refuse concurrent foreground resumes too. Two prompts against one
    // AgentSession race its message collector and overwrite the shared record;
    // callers can retry after the existing turn settles.
    if (record.status === "running" || record.status === "queued") return undefined;

    // Foreground resume: run inline and return the settled record. Replace the
    // old controller so `/agents stop` can reach this new turn, and bridge the
    // caller's signal into the same controller rather than choosing one abort
    ...
    // `waitForAll()` must see an inline resume while it is in flight; the old
    // implementation left `record.promise` pointing at the prior settled run.
    record.promise = run.then(() => "");
    await run;

    // Keep detached nested children alive after a successful resumed turn, but
    // stop the branch when this owner failed or was explicitly stopped.
    this.abortOwnedChildrenIfTerminated(record);
  ...
  private startResume(
    ...
    if (!session) {
      record.status = "error";
      record.error = "Agent session is no longer available.";
      record.completedAt = Date.now();
      if (!this.disposed) {
        try { this.onComplete?.(record); } catch { /* reporting must not strand the record */ }
      }
    ...
    // controller left over from the previous settled turn.
    const abortController = new AbortController();
    record.abortController = abortController;
    ...
    try { this.onStart?.(record); } catch { /* UI observers must not strand the run */ }
    ...
    // A resume can be queued after its session already exists. Steers received
    // while it waited were held by `steer()` and must be injected only after
    // the prompt has been submitted, when AgentSession can accept them.
    if (record.pendingSteers?.length) {
    ...
    record.promise = promise;
    // Queue draining awaits this promise; immediate callers intentionally ignore
    // it because the resume is detached, but the returned value keeps the pool
    // slot charged until the run's settle path has released it.
   ...
   * its current tool execution and appears as a user message. A queued record
   * is held even when it already has a session (background resumes), because
   * that session is not processing the new prompt yet. Pending messages are
   * flushed when the run starts. Returns false if the agent can't accept
   * steering (unknown id, or no longer running/queued).
   */
  ...
  private removeRecord(id: string, record: AgentRecord): void {
    record.queuedAbortCleanup?.();
    record.queuedAbortCleanup = undefined;
    // A completed parent may still have detached nested children. Session
    // cleanup/switch or TTL eviction is an explicit ownership teardown, so stop
    // those children before removing the only session that can receive results.
  ...
  async dispose(pi?: ExtensionAPI): Promise<void> {
    ...
    // Stop model work before detaching records/sessions. Clearing the maps alone
    // would leave in-flight requests running against disposed sessions, burn
    // tokens during shutdown, and let late settle callbacks target a dead host.
    this.abortAll();
