...
import { safeReadFile } from "./memory.js";
...
function readField(path: string): string[] | undefined {
  ...
  try {
    ...
    if (Array.isArray(raw?.enabledModels)) {
      // Hand-edited settings can contain nulls/numbers. Ignore malformed
      // entries instead of letting scope validation throw on `pattern.trim()`.
      return raw.enabledModels.filter((entry: unknown): entry is string => typeof entry === "string");
    }
  } catch {
    /* corrupt file — silent */
  }
...
export function resolveEnabledModels(
  ...
  const available = (Array.isArray(candidates) ? candidates : []).filter(
    (model: unknown): model is ModelEntry =>
