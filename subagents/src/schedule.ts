 ...
 *     automatic `subagent-notification` path. No new delivery code.
 */
...
export class SubagentScheduler {
  ...
  private executeJob(id: string): void {
    ...
    if (job.model) {
      try {
        ...
        // A stale/partial registry should fall back to the agent's normal model
        // selection rather than escape from a timer callback.
      }
    }
    ...
    const finalize = (status: "success" | "error") => {
      try {
        ...
        // Persist lastRun before asking getNextRun(). Interval scheduling derives
        // the next timestamp from lastRun; computing it first reused the previous
        // run's timestamp and made every displayed interval nextRun stale by one
        ...
        store.update(id, { nextRun: next });
      } catch (err) {
        // Completion is already known to AgentManager. Persistence is auxiliary
        // state; report the failure without creating an unhandled timer/promise
        // rejection or preventing later interval ticks.
        this.emit({ type: "error", jobId: id, error: err instanceof Error ? err.message : String(err) });
      }
    };
  ...
  private emit(event: ScheduleChangeEvent): void {
    try {
      ...
      // Cross-extension listeners are observers; a throwing listener must not
      // escape a cron/setInterval callback or stop future scheduled runs.
    }
  }
  ...
  /** Largest delay accepted by Node's setTimeout/setInterval API. */
  private static readonly MAX_TIMER_MS = 2_147_483_647;

  /** Convert a digit+unit duration, rejecting zero, overflow and timer clamp values. */
  private static parseDuration(s: string, prefix: "" | "+"): number | null {
    const pattern = prefix === "+" ? /^\+(\d+)(s|m|h|d)$/ : /^(\d+)(s|m|h|d)$/;
    const m = s.match(pattern);
    if (!m) return null;
    const amount = Number(m[1]);
    const unitMs = { s: 1_000, m: 60_000, h: 3_600_000, d: 86_400_000 }[m[2] as "s" | "m" | "h" | "d"];
    const ms = amount * unitMs;
    return Number.isSafeInteger(ms) && ms > 0 && ms <= SubagentScheduler.MAX_TIMER_MS ? ms : null;
  }

  /** "+10s"/"+5m"/"+1h"/"+2d" → ISO timestamp. */
  static parseRelativeTime(s: string): string | null {
    const ms = SubagentScheduler.parseDuration(s, "+");
    return ms === null ? null : new Date(Date.now() + ms).toISOString();
  }

  /** "10s"/"5m"/"1h"/"2d" → milliseconds. */
  static parseInterval(s: string): number | null {
    return SubagentScheduler.parseDuration(s, "");
  }
}
