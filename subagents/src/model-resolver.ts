...
export function resolveModel(
  ...
  if (queryInput === "") return 'Model name cannot be empty.';
  // Available models (those with auth configured). A minimal test/programmatic
  // registry may expose only find(); treat its catalog as empty rather than
  // throwing while resolving a user-supplied optional pin.
  const candidates = registry.getAvailable?.() ?? registry.getAll?.() ?? [];
  const all = (Array.isArray(candidates) ? candidates : []).filter(
    (m: unknown): m is ModelEntry =>
      !!m && typeof m === "object" &&
  ...
  const slashIdx = queryInput.indexOf("/");
  if (slashIdx !== -1) {
    const provider = queryInput.slice(0, slashIdx);
    const modelId = queryInput.slice(slashIdx + 1);
    if (availableSet.has(queryInput.toLowerCase())) {
  ...
  for (const m of all) {
    ...
    const name = normalize(typeof m.name === "string" ? m.name : "");
  ...
  if (slashIdx !== -1) {
    const bare = resolveModel(queryInput.slice(slashIdx + 1), registry);
  ...
  const modelList = all
    ...
    .sort()
    .join("\n");
  return `Model not found: "${queryInput}".\n\nAvailable models:\n${modelList}`;
}
