...
export function writeInitialEntry(path: string, agentId: string, prompt: string, cwd: string): void {
  ...
  // Transcript output is diagnostic/UI state, never a reason to fail an agent
  // that has already started. The streaming path is best-effort too; keeping
  // the same boundary here prevents a read-only or full /tmp from orphaning a
  // running child behind a failed Agent tool call.
  try {
    writeFileSync(path, JSON.stringify(entry) + "\n", "utf-8");
  } catch {
    // The child result and session remain usable without a transcript file.
  }
}
  ...
  const flush = () => {
    ...
    while (writtenCount < messages.length) {
      ...
      try {
        ...
        // Advance only after the append succeeds. On a transient ENOSPC or
        // permission error, stop this flush and retry the same message on the
        // next event/cleanup instead of silently losing it (and avoid spinning
        // forever on the failed entry).
        writtenCount++;
      } catch {
        break;
      }
