 ...
 *
 * Tools:
 *   Agent             — LLM-callable: spawn a background sub-agent
...
import { defineTool, getAgentDir, getSettingsListTheme, keyHint, type ExtensionAPI, type ExtensionCommandContext, type ExtensionContext } from "@earendil-works/pi-coding-agent";
 ...
 * Background agents use this for the live widget and FleetView.
 */
  ...
  const callbacks = {
    onToolActivity: (activity: { type: "start" | "end"; toolName: string }) => {
      if (activity.type === "start") {
        state.activeTools.set(`${activity.toolName}_${++activitySequence}`, activity.toolName);
      } else {
...
/**
 * Format the model-facing completion payload.
 *
 * This follows OpenCode's Task contract: the parent receives one compact task
 * envelope containing the child's final assistant text in `task_result` (or
 * `task_error`). The JSONL transcript is a UI/debug artifact and must never be
 * included here — otherwise the parent may mistake its path for the answer and
 * read a huge stream of intermediate events instead of using the child's final
 * summary.
 */
function formatTaskNotification(record: AgentRecord, showCost = false): string {
  ...
  // content sent to the parent model.
  ...
  const summary = completed
    ? `Background task completed: ${record.description}`
    : `Background task failed: ${record.description}`;

  return [
    `<task id="${escapeXml(record.id)}" state="${state}">`,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer<NotificationDetails>(
    ...
    (message, { expanded }, theme) => {
      ...
      // This is the exact payload inserted into the parent session. Keep it as
      // the expansion source instead of NotificationDetails.resultPreview:
      // the latter is intentionally bounded for the collapsed card, while
      // Ctrl+O must expose everything the parent model actually received.
      ...
      function renderOne(d: NotificationDetails, includePayload: boolean): string {
        ...
        // Match pi's ToolExecutionComponent: notification rows use the
        // success/error tool background rather than the generic custom-message
        // purple. The returned Box also keeps that tint around every wrapped
        // line, including padding.
        let line = `${icon} ${theme.bold(d.description)} ${isError ? theme.fg("error", statusText) : theme.fg("success", statusText)}`;
      ...
      const background = groupIsError ? "toolErrorBg" : "toolSuccessBg";
  ...
  function emitIndividualNudge(record: AgentRecord) {
    ...
    const notification = formatTaskNotification(record, showCost);
    ...
    // `followUp` waits until the parent finishes its entire current turn. A
    // completion is an urgent new input, so `steer` queues it before the next
    // parent LLM call and interrupts the parent's planned continuation.
  ...
  const groupJoin = new GroupJoinManager(
    (records, partial) => {
      ...
      scheduleNudge(groupKey, () => {
        ...
        const notifications = unconsumed.map(r => formatTaskNotification(r, showCost)).join('\n\n');
        const label = partial
  ...
  const manager = new AgentManager((record) => {
    ...
    // handled by the workflow host instead and have no parentAgentId.
    if (!isTopLevelAgent(record)) {
      const owner = record.parentAgentId ? manager.getRecord(record.parentAgentId) : undefined;
      if (owner?.session) {
        void owner.session.sendCustomMessage({
          customType: "subagent-notification",
          content: formatTaskNotification(record, showCost),
          display: false,
    ...
    // Emit lifecycle event based on terminal status. A third-party listener is
    // not allowed to suppress the completion notification by throwing.
    ...
    } catch { /* lifecycle observers are best-effort */ }

    // Persist final record for cross-extension history reconstruction. Session
    // replacement/readonly hosts can reject this append; the model-facing
    // completion still has to be delivered in that case.
    try {
    ...
    } catch { /* persistence is best-effort; notification is not */ }

    // Skip notification if an external RPC consumer already consumed it
  ...
  const spawnResolved = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    //
    // Turn limits are disabled, so every programmatic spawn gets an unbounded
    // tracker as well as an unbounded runner. The turn count remains useful UI
    // telemetry (`↻3`) but never carries a ceiling or abort policy.
    const { state, callbacks } = createActivityTracker();
    // RPC callers are allowed to omit the optional payload, but the manager's
    // renderer needs a stable string description (escapeXml is intentionally
    // strict). Fill a safe default before the record is created and discard a
    // malformed name rather than letting handle allocation throw after enqueue.
    const spawnOptions = {
      ...(options ?? {}),
      description:
    ...
    const id = manager.spawn(piRef, ctxRef, dispatch.type, prompt, spawnOptions);
  ...
  const spawnTopLevel = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    // Every spawn through here is DETACHED and pooled as background work — the
    // caller gets an id back and receives the same automatic full-result
    // notification as an Agent-tool call. Do not let an RPC/registry caller set
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (!rpcHandle) {
      rpcHandle = registerRpcHandlers({
        ...
        manager: {
          ...
          consumeResult: (id) => {
            ...
            // A running agent has no result to consume, and its completion
            // notification is still the caller's only signal that it finished.
            // Workflow-owned children are also private to their workflow; only
            // a top-level result may be consumed by an external RPC caller.
            if (!record || !isTopLevelAgent(record)) return false;
      ...
      try { pi.events.emit("subagents:ready", {}); } catch { /* observer */ }
    }
  ...
  pi.on("input", async (event, ctx) => {
    ...
    try {
      // Nothing else to pass: runAgent resolves model and thinking from the
      // agent's own config when the spawn omits them, and the manager's
      // onStart/onComplete callbacks own the widget, the fleet list and the
  ...
  // Every completed subagent result is useful independently. Keep grouping as an
  // explicit opt-in, rather than holding the default result behind a sibling or
  // the 30-second group timeout.
  let defaultJoinMode: JoinMode = 'async';
  function getDefaultJoinMode(): JoinMode { return defaultJoinMode; }
  function setDefaultJoinMode(mode: JoinMode) { defaultJoinMode = mode; }

  // Kept as legacy in-memory configuration for compatibility with persisted
  // settings, but top-level and nested Agent execution are always detached in
  // this fork. The Agent tool never reads this as a foreground escape hatch.
   ...
   *
   * Shared by the Agent tool's detached resume and the `@handle message`
   * prompt mention — they differ only in how they report the outcome. Returns the record, or undefined when the manager refused because
  ...
  async function startBackgroundResume(
    ...
    if (opts.outputTranscript) {
      try {
        ...
        // Transcript output is optional diagnostics; a read-only/full temp dir
        // must not prevent a background resume from starting.
        existing.outputFile = undefined;
      }
    } else {
      // A setting/frontmatter change can disable transcripts after an earlier
      // run already created one. Stop its listener and clear the path, or the
      // resumed run would silently keep writing despite output_transcript:false.
      if (existing.outputCleanup) {
        try { existing.outputCleanup(); } catch { /* ignore */ }
    ...
    // background spawns in that same turn keep going.
  ...
  applyAndEmitLoaded(
    {
      ...
      setDefaultJoinMode,
      // Background-only fork: retain the legacy setting key but always keep
      // top-level Agent calls detached.
      setBackgroundByDefault: () => setBackgroundByDefault(true),
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const agentTool = defineTool({
    ...
    renderResult(result, { expanded, isPartial }, theme, renderContext) {
      ...
      if (details.status === "completed" || details.status === "steered") {
        ...
        if (expanded) {
          ...
          if (resultText) {
            ...
            if (resultText.split("\n").length > 50) {
              line += "\n" + theme.fg("muted", "  ... (full result was delivered in the completion notification)");
            }
          }
        } else {
          const doneText = isSteered ? "Completed after steering" : "Done";
      ...
      // its own — the error wording below must not be the catch-all.
      if (details.status !== "error" && details.status !== "aborted") {
        return new Text(text, 0, 0);
      }

      // ---- Error / Aborted (legacy records or external stop) ----
      const s = stats(details);
      ...
      if (details.status === "error") {
        ...
        line += "\n" + theme.fg("warning", "  ⎿  Aborted (legacy run)");
      }
    ...
    execute: async (toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      // This fork exposes background execution only. Keep the resolved config
      // for compatibility with custom agent metadata, but never expose or honor
      // a foreground escape hatch at the top-level Agent tool.
      ...
      const attachTranscript = (rec: AgentRecord | undefined, agentId: string): void => {
        ...
        try {
          ...
          // Transcript output is optional diagnostics; a read-only/full temp dir
          // must not turn an already-started background agent into a failed call.
      ...
      // Turn limits are disabled in this fork. Any legacy limit field in an
      // old agent file or call is ignored and never reaches the runner/UI.
      ...
      if (params.resume) {
        ...
        // Resumes are detached too: the new turn's complete result is sent
        // automatically through the same completion notification as a fresh spawn.
        const id = existing.id;
        ...
        if (existing.status === "running" || existing.status === "queued") {
          return textResult(
            ...
            `Use steer_subagent to send it a message mid-run; its final result will be sent automatically.`,
          );
        }
      ...
      // Background execution (the only top-level Agent mode)
      {
        const { state: bgState, callbacks: bgCallbacks } = createActivityTracker();
  ...
  /**
   * Hand a finished run back to the model through the same automatic completion
   * channel as a background agent. It is delivered as a steer so a running
   * parent processes the result before its next planned LLM call.
   */
  ...
  function runWorkflowFlag(ctx: ExtensionContext): void {
    ...
    const resolved = resolveWorkflowScript({ scriptPath: path }, ctx.cwd);
    if (!resolved.ok) {
      report(resolved.message, "warning");
      return;
    }
    const script = resolved.script;
    const scriptPath = resolved.scriptPath ?? path;
    ...
    const task = createWorkflowTask({ id: workflowRunId(), script, scriptPath, meta });
    ...
    void runWorkflowTask(ctx, task).then(() => {
      ...
      try {
        ...
        // Session persistence/delivery is auxiliary to the completed workflow;
        // a readonly or shutting-down host must not create an unhandled rejection.
      }
      widget.update();
      fleet.update();
    }).catch(() => {
      // runWorkflowTask is intentionally non-throwing; retain a final guard for
      // host/UI failures in detached startup paths.
    });
  }

  // Result retrieval is intentionally absent: completion notifications carry
  // the complete final assistant message automatically.
  ...
  registerToolReportingUsage(defineTool({
    ...
    parameters: Type.Object({
      agent_id: Type.String({
        description: "The agent ID to steer (must be currently running or queued). The agent's handle also works — its `name` if you gave it one, otherwise its type (`explore`, `explore-2`).",
      }),
      message: Type.String({
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, _ctx) => {
      ...
      if (record.status !== "running" && record.status !== "queued") {
        return textResult(`Agent "${params.agent_id}" is not running or queued (status: ${record.status}). Cannot steer a settled agent.`);
      }
      if (record.status === "queued" || !record.session) {
        // A queued resume already has a session object, but it is not processing
        // the new prompt yet. Hold the steer until the queued run starts rather
        // than calling session.steer() against an idle session.
        if (!record.pendingSteers) record.pendingSteers = [];
        record.pendingSteers.push(params.message);
        pi.events.emit("subagents:steered", { id: record.id, message: params.message });
        return textResult(`Steering message queued for agent ${record.id}. It will be delivered when the agent starts.`);
      }
...
- Top-level Agent calls always run in the background; completion results are sent automatically to the parent session
...
Write the file using the write tool. Only write the file, nothing else.`;
  ...
  function snapshotSettings() {
    return {
      ...
      // Kept in the persisted shape for backwards compatibility; top-level
      // Agent calls are always detached in this fork.
  ...
  const NUMERIC_IDS = new Set([
    "maxConcurrent", "maxSubagentDepth",
  ]);
