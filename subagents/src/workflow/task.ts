...
/** Format the model-facing workflow completion using OpenCode's task shape. */
export function formatWorkflowNotification(task: WorkflowTask, now = Date.now()): string {
  ...
  const result = workflowResultText(task) || task.error || "No output.";
  const summary = completed
    ? `Background task completed: ${task.workflowName ?? task.id}`
    : `Background task failed: ${task.workflowName ?? task.id}`;
  const progress = `${totals.done}/${totals.total} agents`;
  return [
    `<task id="${escapeXml(task.id)}" state="${state}">`,
    `<summary>${escapeXml(summary)} (${progress})</summary>`,
    `<${resultTag}>`,
    escapeXml(result),
    `</${resultTag}>`,
