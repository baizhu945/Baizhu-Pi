...
import { hasSymlinkComponent, isSymlink, isUnsafeName, safeReadFile } from "../memory.js";
...
export function readSavedWorkflow(name: string, cwd: string): SavedWorkflow {
  ...
  for (const root of roots) {
    if (isSymlink(root) || hasSymlinkComponent(root)) continue; // reject a symlinked root entirely, as skill-loader does
...
export function resolveWorkflowSource(
  ...
  if (path !== undefined && path !== "") {
    ...
    try {
      if (statSync(resolved).size > MAX_SCRIPT_LENGTH) {
        return {
          ok: false,
          message: `Workflow script "${resolved}" is over the limit of ${MAX_SCRIPT_LENGTH} characters.`,
        };
      }
      // Use the same symlink-aware reader as named workflows. Explicit paths
      // may be absolute by contract, but a symlinked file/parent must not turn
      // a seemingly local workflow into an arbitrary target read.
      const script = safeReadFile(resolved);
      if (script === undefined) {
        return {
          ok: false,
          message: `Could not read workflow script "${resolved}" (missing, unreadable, or symlinked).`,
        };
      }
      return { ok: true, script, path: resolved };
    } catch (err) {
      return {
        ok: false,
...
export function listSavedWorkflows(cwd: string): string[] {
  ...
  for (const root of savedWorkflowRoots(cwd)) {
    if (!existsSync(root) || isSymlink(root) || hasSymlinkComponent(root)) continue;
    let entries: string[];
    try {
