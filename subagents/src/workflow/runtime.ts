...
export function workflowConcurrency(cpuCount: number = cpus().length): number {
  const cores = Number.isFinite(cpuCount) ? Math.max(1, Math.floor(cpuCount)) : 1;
...
class Semaphore {
  private active = 0;
  private readonly waiters: ((acquired: boolean) => void)[] = [];

  constructor(private readonly limit: number) {}

  /** `false` means the waiter was released by run shutdown, not given a permit. */
...
export async function runWorkflow(options: RunWorkflowOptions): Promise<WorkflowRunResult> {
  ...
  // Public runtime callers may bypass the tool schema. Clamp malformed caps so
  // a zero/negative semaphore cannot leave every workflow call waiting forever,
  // while zero remains a meaningful "allow no agents/items/nested calls" cap.
  const positiveInt = (value: number | undefined, fallback: number): number =>
    Number.isSafeInteger(value) && value !== undefined ? Math.max(1, value) : fallback;
  const nonNegativeInt = (value: number | undefined, fallback: number): number =>
    Number.isSafeInteger(value) && value !== undefined ? Math.max(0, value) : fallback;
  const agentCap = nonNegativeInt(options.agentCap, WORKFLOW_AGENT_CAP);
  const itemCap = nonNegativeInt(options.itemCap, WORKFLOW_ITEM_CAP);
  const nestedCap = nonNegativeInt(options.nestedCap, WORKFLOW_NESTED_CAP);
  const semaphore = new Semaphore(positiveInt(options.concurrency, workflowConcurrency()));
  ...
  const recordJournal = options.journal?.append
    ? (entry: WorkflowJournalEntry): void => {
        try { options.journal?.append?.(entry); } catch { /* journal is auxiliary state */ }
      }
    : undefined;
  /**
  ...
  try {
    ...
    skip: index => {
      ...
      if (live.started) {
        try { host.abortAgent(live.agentId); } catch { /* best-effort control */ }
      } else live.wake?.();
      return true;
    },
    retry: index => {
  ...
  } catch { /* control surface is an optional UI observer */ }
  ...
  return await new Promise<WorkflowRunResult>(resolve => {
    const emit = (entries: WorkflowEntry[]) => {
      ...
      try { options.onProgress?.(entries); } catch { /* UI observer */ }
    };
    ...
    const respond = (callId: number, ok: boolean, value?: unknown, error?: string, fatal?: boolean) => {
      ...
      try {
        ...
        // The worker may have exited between the settled check and this send.
        // Its error/exit handler owns finalization; never leak an exception from
    ...
    const finish = (result: Omit<WorkflowRunResult, "meta" | "progress" | "agentCount" | "replayedCount">) => {
      ...
      for (const agentId of inflight) {
        try { host.abortAgent(agentId); } catch { /* best-effort shutdown */ }
      }
      inflight.clear();
      semaphore.drain();
    ...
    async function handleAgent(callId: number, payload: AgentCallPayload): Promise<void> {
      ...
      try {
        for (;;) {
          ...
          if (!acquired || aborted || settled) {
            // A drained waiter never owned a permit, so do not release one it
            // did not acquire (which would corrupt the semaphore count).
      ...
      switch (message.type) {
        ...
        case "call":
          if (message.method === "workflow") {
            void handleLoadWorkflow(message.callId, message.payload as WorkflowScriptRef).catch(error => {
              respond(message.callId, false, undefined, error instanceof Error ? error.message : String(error), true);
            });
            break;
          }
          ...
          void handleAgent(message.callId, message.payload as AgentCallPayload).catch(error => {
            respond(message.callId, false, undefined, error instanceof Error ? error.message : String(error), true);
          });
          break;
        case "complete": {
          ...
          try {
            finish({ status: "completed", value: JSON.parse(message.resultJson) });
          } catch {
            finish({ status: "failed", error: "Workflow returned invalid JSON." });
          }
          break;
        }
        case "error":
