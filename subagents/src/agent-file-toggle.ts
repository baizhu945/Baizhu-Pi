...
export function serializeAgentFile(cfg: AgentConfig): string {
  ...
  // Top-level and nested execution are background-only in this fork. Do not
  // emit the legacy `run_in_background` field: it would suggest that a custom
  // agent can opt into a foreground path which is no longer exposed.
