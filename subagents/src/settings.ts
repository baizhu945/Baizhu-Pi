...
export interface SubagentsSettings {
  ...
  /** Default is `async`: each completed result is delivered independently and immediately. */
  defaultJoinMode?: JoinMode;
  /**
   * Legacy compatibility key. This fork exposes background execution only, so
   * its runtime always keeps top-level and nested Agent calls detached and
   * ignores attempts to turn this value off.
   */
...
export function applyAndEmitLoaded(
  ...
  try { emit("subagents:settings_loaded", { settings }); } catch { /* observer */ }
  return settings;
}
...
export function saveAndEmitChanged(
  ...
  try { emit("subagents:settings_changed", { settings: snapshot, persisted }); } catch { /* observer */ }
