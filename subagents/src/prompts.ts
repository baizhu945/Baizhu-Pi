...
export interface PromptExtras {
   ...
   * output IS read by a human — through the completion notification, FleetView
   * and the conversation viewer — so terse raw data would be the wrong answer there.
   */
...
Return only the answer, in exactly the shape the prompt asks for — no preamble, no summary of what you did, no offer to continue.
  ...
  // This is the ordinary subagent result contract. Transcript files remain a
  // human/debug surface; the parent must receive the child's closing assistant
  // message itself, just as OpenCode's Task tool injects its final text.
  const resultContract = extras?.workflowChild
    ? ""
    : `\n\n<subagent_result_contract>\nYour final assistant message is the result delivered to the parent agent. Return the completed answer in the format requested by the task; for research or coding work, include a concise summary of concrete findings. Do not direct the parent to read a transcript or output file instead of returning the result.\n</subagent_result_contract>`;
