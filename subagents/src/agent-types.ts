...
/**
 * Return the effective config exposed to every spawn path.
 *
 * Explore is deliberately parent-model-only in this fork. Keep that invariant
 * at the registry boundary as well as in DEFAULT_AGENTS: it protects against a
 * stale embedded default (or a future upstream merge) reintroducing the old
 * Haiku pin. A user-authored agent that overrides Explore is not marked
 * `isDefault`, so its explicit model remains authoritative.
 */
function effectiveAgentConfig(config: AgentConfig | undefined): AgentConfig | undefined {
  if (
    config?.isDefault === true &&
    config.name.toLowerCase() === "explore" &&
...
export function getAgentConfigIn(registry: Map<string, AgentConfig>, name: string): AgentConfig | undefined {
  ...
  return effectiveAgentConfig(key ? registry.get(key) : undefined);
}
