...
function loadFromDir(dir: string, agents: Map<string, AgentConfig>, source: "project" | "global", strict: boolean): void {
  ...
  let files: string[];
  try {
    // Directory enumeration order is filesystem-dependent. Sort so two files
    // declaring the same frontmatter name resolve deterministically, making the
    // documented last-load-wins rule reproducible across machines.
