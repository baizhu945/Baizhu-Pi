...
export interface SpawnCapable {
  ...
  /**
   * Mark a settled agent's result as read by an external RPC consumer,
   * suppressing the automatic completion notification. False when there is no
   * such agent, or it has not settled yet.
   */
  ...
  return events.on(channel, async (raw: unknown) => {
    ...
    try {
      ...
      try {
        ...
        // A throwing reply listener is outside this handler's control; never
        // turn it into an unhandled rejection in the event-bus callback.
      }
    } catch (err: any) {
      try {
        ...
        // Reply delivery is best-effort; the original RPC error is already
        // contained and must not take down the host.
      }
    }
  });
}
...
export function registerRpcHandlers(deps: RpcDeps): RpcHandle {
  ...
  const unsubSpawn = handleRpc<{ requestId: string; type: string; prompt: string; options?: any }>(
    events, "subagents:rpc:spawn", async ({ type, prompt, options }) => {
      if (typeof type !== "string" || type.trim() === "") throw new Error("Agent type must be a non-empty string");
      ...
      if (override != null) {
        ...
        if (
          ...
          (typeof override !== "object" || override === null ||
            typeof override.provider !== "string" || typeof override.id !== "string")
        ) {
          throw new Error("Model override must be a provider/model string or model object");
        }
