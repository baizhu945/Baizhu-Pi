 ...
 *   - and it uses the fork's background-only `Agent` path. A synchronous agent
 *     result would be returned only to this throwaway clone, so completion
...
export async function runMentionClone(opts: MentionCloneOptions): Promise<MentionCloneResult> {
  ...
  const cloneAgentTool: ToolDefinition = {
    ...
    execute: (_cloneToolCallId, params, signal, onUpdate, _cloneCtx) => {
      ...
      spawned = true;
      // The undefined tool-call id + main ctx are the important parts here:
      // the fork's Agent implementation is background-only, so its completion
      // notification is delivered to the real session rather than this clone.
      return agentTool.execute(undefined as never, params as typeof params, signal, onUpdate, ctx);
    },
  };
  ...
  try {
    ...
    try { session?.dispose?.(); } catch { /* a clone teardown is best-effort */ }
  }
