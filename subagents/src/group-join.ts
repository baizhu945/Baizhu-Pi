...
export class GroupJoinManager {
  ...
  private onTimeout(group: AgentGroup): void {
    ...
    // Deliver what we have. Delivery is an observer callback; even if a
    // renderer/sendMessage fails, the straggler state must still be installed
    // so the remaining agents can complete and the timer cannot escape.
    try {
  ...
  private deliver(group: AgentGroup, partial: boolean): void {
    ...
    // Always release the group mapping, even when delivery throws; otherwise
    // completed records remain pinned until the manager is disposed.
